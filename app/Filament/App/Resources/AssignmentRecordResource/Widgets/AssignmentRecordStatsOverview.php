...
namespace App\Filament\App\Resources\AssignmentRecordResource\Widgets;
...
class AssignmentRecordStatsOverview extends BaseWidget
{
    protected function getStats(): array
    {
        $currentMtd = AssignmentRecord::query()->whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])->count();
        $lastMtd = AssignmentRecord::query()->whereBetween('created_at', [now()->subMonth()->startOfMonth(), now()->subMonth()->endOfMonth()])->count();
        $percentageMtd = (($currentMtd - $lastMtd) / ($lastMtd === 0 ?: 1));

        $currentQtd = AssignmentRecord::query()->whereBetween('created_at', [now()->startOfQuarter(), now()->endOfQuarter()])->count();
        $lastQtd = AssignmentRecord::query()->whereBetween('created_at', [now()->subQuarter()->startOfQuarter(), now()->subQuarter()->endOfQuarter()])->count();
        $percentageQtd = (($currentQtd - $lastQtd) / ($lastQtd === 0 ?: 1));

        $currentYtd = AssignmentRecord::query()->whereBetween('created_at', [now()->startOfYear(), now()->endOfYear()])->count();
        $lastYtd = AssignmentRecord::query()->whereBetween('created_at', [now()->subYear()->startOfYear(), now()->subYear()->endOfYear()])->count();
        $percentageYtd = (($currentYtd - $lastYtd) / ($lastYtd === 0 ?: 1));

        $mtd = Trend::model(AssignmentRecord::class)
            ->between(
        ...
        $qtd = Trend::model(AssignmentRecord::class)
            ->between(
        ...
        $ytd = Trend::model(AssignmentRecord::class)
            ->between(
